zenergyApp.controller("usersPageController", ["$scope", "$resource", "$uibModal","$location", function ($scope, $resource, $uibModal,$location) {
    ...
    if ($scope.isAdmin()) {
        ...
        $scope.validate = function () {
            angular.forEach($scope.users, function (u, key) {
                ...
                if (u.memberChecked)
                    Member.save({ userId: u.userId, dateMembership: new Date() });//member role added
                else
